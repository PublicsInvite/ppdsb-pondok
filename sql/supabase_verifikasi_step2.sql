-- =====================================================
-- STEP 2: Tambah Kolom & Constraint (RUN SETELAH STEP 1)
-- =====================================================
-- Run script ini SETELAH step 1 berhasil
...
-- 3. Tambah kolom alasan (catatan verifikasi)
ALTER TABLE pendaftar 
ADD COLUMN IF NOT EXISTS alasan TEXT;

-- 4. Update constraint untuk statusberkas
-- Drop constraint lama jika ada
ALTER TABLE pendaftar 
DROP CONSTRAINT IF EXISTS pendaftar_statusberkas_check;

-- Buat constraint baru dengan status UPPERCASE
ALTER TABLE pendaftar 
...
-- 6. Tambah index untuk performa query
...
-- 7. Update RLS policy untuk admin dapat update verifikasi
...
-- 8. Buat function untuk auto-set verifiedat saat status berubah
...
-- 10. Optional: Buat view untuk laporan verifikasi
-- Drop view lama jika ada
DROP VIEW IF EXISTS v_pendaftar_verifikasi;

-- Create view baru dengan struktur lengkap
...
-- 11. Grant akses view ke anon untuk public read
